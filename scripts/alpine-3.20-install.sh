...
arm|armv7l) cpu=armv7;;
*) echo "目前不支持$(uname -m)架构" && exit 1
esac
if [[ ! -f "$HOME/storage/downloads/$tarball" ]]; then
echo "下载 '$osname' 镜像文件 '$tarball'，请耐心等待"
dlurl="https://raw.bunnyxyz.eu.org/https://github.com/mlabalabala/resource/blob/main/pkg/alpine-${cpu}-3.20_rootfs.tar.gz"
curl -f#SLk $dlurl -o $HOME/storage/downloads/$tarball
fi
echo '正在解压镜像 请耐心等待'
mkdir -p $HOME/ternux/$osname/$folder
proot --link2symlink tar -vzxf $HOME/storage/downloads/$tarball -C $HOME/ternux/$osname/$folder --exclude="dev"||:
...
command+=" HTTP_PROXY=${proxy_addr}"
command+=" HTTPS_PROXY=${proxy_addr}"
command+=" HOME=/root"
...
bash $HOME/ternux/$osname/$shname 'apk update;apk upgrade;apk add grep curl nodejs openssl bash jq;exit'
