...
function df4f63(key, iv, data) {
    const new_iv = Crypto.enc.Utf8.parse(iv); // 长度必须为16
    const new_key = Crypto.enc.Utf8.parse(key); // 长度为16 -> AES-128
    let decrypted = Crypto.AES.decrypt(data, new_key, {
        iv: new_iv,
...
function ee8e00(key, iv, data) {
    const new_iv = Crypto.enc.Utf8.parse(iv);
    const new_key = Crypto.enc.Utf8.parse(key);
    let encrypted = Crypto.AES.encrypt(data, new_key, {
        iv: new_iv,
        mode: Crypto.mode.CBC,
        padding: Crypto.pad.Pkcs7
    });
...
function formatTitleSt(st) {
    // return st.replace(/\s+/g,"").replaceAll(/[\r\n]/g,"").replaceAll("#","※").replaceAll("$","￥").replaceAll("&","AND")
    return st.trim().replaceAll(/[\r\n]/g,"").replaceAll("#","※").replaceAll("$","￥").replaceAll("&","AND")
}

function handleBase64ImgSt(st) {
